...
export class PostService {
  ...
  getPosts(): Observable<Post[]> {
    ...
    return this._http
      .get(`${this._backendUri}/posts?publicationDate_lte=${fechaAhora}&_sort=publicationDate&_order=desc`)
  ...
  getUserPosts(id: number): Observable<Post[]> {
    ...
    let fechaAhora = Date.now();
    return this._http
      .get(`${this._backendUri}/posts?author.id=${id}&publicationDate_lte=${fechaAhora}&_sort=publicationDate&_order=desc`)
      .map((response: Response): Post[] => Post.fromJsonToList(response.json()));
  }
  ...
  getCategoryPosts(id: number): Observable<Post[]> {
    ...
    return this._http
      ...
      .map((response: Response): Post[] => {
        return Post.fromJsonToList(response.json().filter((p:Post): boolean => { 
